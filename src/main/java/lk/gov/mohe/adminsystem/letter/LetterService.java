...
public class LetterService {
  ...
  private Specification<Letter> buildScopeSpec(
    ...
    if (authorities.contains("letter:own:manage")) {
  ...
  public void returnFromUser(Integer letterId, Integer currentUserId, String reason) {
    ...
    Map<String, Object> eventDetails =
        Map.of(
            "newStatus",
            StatusEnum.RETURNED_FROM_OFFICER,
            "userId",
            currentUserId,
            "reason",
            reason);
  ...
  public void addAttachment(
    ...
    if (attachments == null || attachments.length == 0) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Attachments field is required");
    }
  ...
  @Transactional
  public void changeLetterPriority(
      Integer letterId,
    ...
    if (!hasAccessToLetter(letter, userId, divisionId, authorities, "update:priority")) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You do not have permission to change the priority of this letter");
    }

    if (newPriority == null) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Priority is required");
    }
    PriorityEnum previousPriority = letter.getPriority();

    letter.setPriority(newPriority);
    letterRepository.save(letter);

    Map<String, Object> eventDetails =
        Map.of("newPriority", newPriority, "previousPriority", previousPriority);
    createLetterEvent(letter, EventTypeEnum.CHANGE_PRIORITY, eventDetails);
  }
  ...
  private boolean hasAccessToLetter(
    ...
    boolean hasOwnAccess =
        authorities.contains("letter:own:manage")
            && letter.getAssignedUser() != null
