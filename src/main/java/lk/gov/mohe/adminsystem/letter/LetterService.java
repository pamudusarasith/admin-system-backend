...
public class LetterService {
    ...
    public Page<LetterDto> getAccessibleLetters(
        ...
        if (authorities.contains("letter:division:read")) {
            Specification<Letter> divisionSpec = (root, query, cb) -> cb.equal(root.get("assignedDivision").get("id"),
                    divisionId);
            spec = (spec == null) ? divisionSpec : spec.or(divisionSpec);
        }

        if (authorities.contains("letter:own:read")) {
            Specification<Letter> ownSpec = (root, query, cb) -> cb.equal(root.get("assignedUser").get("id"), userId);
    ...
    public void assignUser(
        ...
        if (letter.getAssignedUser() != null) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "Letter is already assigned to a user");
        }
        ...
        Map<String, Object> eventDetails = Map.of(
                "newStatus", StatusEnum.PENDING_ACCEPTANCE,
