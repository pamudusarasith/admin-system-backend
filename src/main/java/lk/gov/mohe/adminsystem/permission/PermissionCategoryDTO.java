...
public record PermissionCategoryDTO(Integer id, String name, Integer parentId
  ...
  public static PermissionCategoryDTO fromEntity(PermissionCategory category) {
    return new PermissionCategoryDTO(
        ...
        // category.getPermissions().stream().map(PermissionDTO::fromEntity).collect(Collectors.toList()) : List.of()
        );
  }
}
