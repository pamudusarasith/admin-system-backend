...
public class AuthService {
  ...
  public AuthTokensDto refreshAccessToken(RefreshTokenRequestDto refreshTokenRequest) {
    ...
    Optional<RefreshToken> refreshTokenOpt = refreshTokenService.validateRefreshToken(tokenValue);
  ...
  private AccessTokenDto generateAccessToken(User user) {
    if (user == null) {
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Cannot generate token for null user");
    }

    if (user.getRole() == null || user.getRole().getPermissions() == null) {
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "User has invalid role or permissions");
    }
