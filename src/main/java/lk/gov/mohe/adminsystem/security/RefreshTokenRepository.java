...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Integer> {
  ...
  @Query(
      "SELECT rt FROM RefreshToken rt WHERE rt.user = :user AND rt.revoked = false AND rt.expiresAt > :now")
  ...
  @Query(
      "SELECT COUNT(rt) FROM RefreshToken rt WHERE rt.user = :user AND rt.revoked = false AND rt.expiresAt > :now")
